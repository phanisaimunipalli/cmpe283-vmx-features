...
struct capability_info procbasedctls2[28] = {
...
struct capability_info procexitctls[16] = {
...
struct capability_info procentryctls[13] = {
...
struct capability_info procbasedctls3[4] = {
        {0,"LOADIWKEY exiting"},
        {1,"Enable HLAT"},
...
detect_vmx_features(void)
{
	...
	rdmsr(IA32_VMX_EXIT_CTLS, lo, hi);
		...
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(procexitctls, 16, lo, hi);
	...
	pr_info("Entry Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(procentryctls, 13, lo, hi);
